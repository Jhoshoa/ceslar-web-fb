...
const { requireSystemAdmin, requireOwnerOrAdmin } = require('../middleware/permissions.middleware');
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const user = await userService.updateUserRole(req.params.id, systemRole);
      success(res, user);
    } catch (error) {
      ...
      if (!isSystemAdmin(req.user) && !isChurchAdmin(req.user, churchId)) {
        return res.status(403).json({
          ...
          error: {
            ...
            message: 'You must be a system admin or church admin to assign roles.',
          },
        });
      }

      const result = await userService.assignChurchRole(targetUserId, churchId, role);
      success(res, result);
    } catch (error) {
    ...
    try {
      ...
      if (!isSystemAdmin(req.user) && !isChurchAdmin(req.user, churchId)) {
        return res.status(403).json({
          ...
          error: {
            code: 'auth/insufficient-permissions',
            message: 'You must be a system admin or church admin to update roles.',
          },
        });
      }
    ...
    try {
      ...
      if (!isSystemAdmin(req.user) && !isChurchAdmin(req.user, churchId)) {
        return res.status(403).json({
          ...
          error: {
            code: 'auth/insufficient-permissions',
            message: 'You must be a system admin or church admin to remove roles.',
          },
        });
      }
