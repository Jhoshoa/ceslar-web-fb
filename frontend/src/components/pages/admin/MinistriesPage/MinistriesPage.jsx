...
const MinistriesPage = () => {
  ...
  const [dialog, setDialog] = useState({ open: false, ministry: null });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => {
        const desc = row.description?.[lang] || row.description?.es || '';
        return (
  ...
  const openEdit = (ministry) => {
    setForm({
      name: ministry.name?.[lang] || ministry.name?.es || '',
      description: ministry.description?.[lang] || ministry.description?.es || '',
    });
