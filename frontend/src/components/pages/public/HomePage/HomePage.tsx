...
const SectionTitle = ({ title, subtitle, light = false, centered = true, action }: SectionTitleProps) => (
  <Box
    sx={{
      display: 'flex',
    ...
    <Card onClick={onClick} sx={{ cursor: 'pointer', display: 'flex', height: '100%', minHeight: 140 }}>
      ...
      <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', color: 'text.secondary', mb: 0.5 }}>
          ...
          <Typography variant="body2">{event.startTime || '10:00 AM'}</Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', color: 'text.secondary' }}>
  ...
  return (
    <Card
      ...
      sx={{
        ...
        display: 'flex',
        flexDirection: 'column',
        '&:hover .sermon-play': { transform: 'scale(1.1)', bgcolor: 'secondary.main' },
      }}
    >
...
interface MinistryCardItemProps {
  ministry: Ministry;
  onClick: () => void;
}

const MinistryCardItem = ({ ministry, onClick }: MinistryCardItemProps) => {
  const name = typeof ministry.name === 'string' ? ministry.name : ministry.name?.es || '';
  const description = typeof ministry.description === 'string'
    ? ministry.description
    : ministry.description?.es || '';
  ...
  return (
    <Card
      ...
      sx={{
        ...
        p: 3,
        height: '100%',
        '&:hover': { '& .ministry-icon': { bgcolor: 'secondary.main', color: 'primary.main' } },
      }}
    >
      <Box
...
const HomePage = () => {
  ...
  return (
    <Box>
      ...
      <Box sx={{ py: { xs: 8, md: 12 }, bgcolor: 'background.paper' }}>
        <Container maxWidth="lg">
          <SectionTitle
            ...
            centered={false}
            action={{ label: t('common.viewAll', 'Ver todas'), onClick: () => navigate('/churches') }}
          />
            ...
            {loadingChurches
              ...
              : churches.map((church) => (
                  <Grid item xs={12} sm={6} md={3} key={church.id}>
                    <ChurchCardItem church={church} onClick={() => navigate(`/churches/${church.slug}`)} />
                  </Grid>
                ))}
          </Grid>
        </Container>
      ...
      <Box sx={{ py: { xs: 8, md: 12 }, background: 'linear-gradient(135deg, #0D1B4C 0%, #1E3A8A 100%)' }}>
        <Container maxWidth="lg">
          <SectionTitle
            ...
            light
            centered={false}
            action={{ label: t('common.viewAll', 'Ver todos'), onClick: () => navigate('/events') }}
          />
          <Grid container spacing={3}>
            {loadingEvents
              ...
              : events.map((event) => (
                  <Grid item xs={12} md={4} key={event.id}>
                    <EventCardItem event={event} onClick={() => navigate(`/events/${event.id}`)} />
                  </Grid>
                ))}
          </Grid>
        </Container>
      ...
      <Box sx={{ py: { xs: 8, md: 12 }, bgcolor: 'background.default' }}>
        <Container maxWidth="lg">
          <SectionTitle
            ...
            centered={false}
            action={{ label: t('common.viewAll', 'Ver todos'), onClick: () => navigate('/sermons') }}
          />
          <Grid container spacing={3}>
            {loadingSermons
              ? Array.from({ length: 3 }).map((_, i) => (
              ...
              : sermons.map((sermon) => (
                  <Grid item xs={12} sm={6} md={4} key={sermon.id}>
                    <SermonCardItem sermon={sermon} onClick={() => navigate(`/sermons/${sermon.id}`)} />
                  </Grid>
                ))}
          </Grid>
        </Container>
      ...
      <Box sx={{ py: { xs: 8, md: 12 }, bgcolor: 'background.paper' }}>
        <Container maxWidth="lg">
          <SectionTitle
            ...
            centered={false}
            action={{ label: t('common.viewAll', 'Ver todos'), onClick: () => navigate('/ministries') }}
          />
          <Grid container spacing={3}>
            {loadingMinistries
              ? Array.from({ length: 4 }).map((_, i) => (
              ...
              : ministries.map((ministry) => (
                  <Grid item xs={12} sm={6} md={3} key={ministry.id}>
                    <MinistryCardItem ministry={ministry} onClick={() => navigate('/ministries')} />
                  </Grid>
                ))}
          </Grid>
        </Container>
      ...
      >
        ...
        >
          <Box sx={{ width: 8, height: 200, bgcolor: '#0D1B4C', position: 'absolute', left: '50%', transform: 'translateX(-50%)' }} />
          <Box sx={{ width: 120, height: 8, bgcolor: '#0D1B4C', position: 'absolute', top: 50, left: '50%', transform: 'translateX(-50%)' }} />
        </Box>
        ...
        <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <Typography variant="h6" sx={{ color: alpha('#0D1B4C', 0.8), mb: 4, fontWeight: 400 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap' }}>
            <Button
              ...
              sx={{ bgcolor: '#0D1B4C', color: 'white', px: 4, py: 1.5, '&:hover': { bgcolor: '#1E3A8A' } }}
            >
            ...
            <Button
              ...
              sx={{
                ...
                px: 4,
                py: 1.5,
                '&:hover': { borderWidth: 2, borderColor: '#0D1B4C', bgcolor: alpha('#0D1B4C', 0.05) },
              }}
            >
      ...
      <Box sx={{ py: 6, bgcolor: 'background.paper' }}>
        <Container maxWidth="sm" sx={{ textAlign: 'center' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
            <IconButton
              sx={{ bgcolor: alpha('#0D1B4C', 0.08), '&:hover': { bgcolor: '#1877F2', color: 'white' } }}
              size="large"
            >
              <FacebookIcon />
            </IconButton>
            <IconButton
              sx={{ bgcolor: alpha('#0D1B4C', 0.08), '&:hover': { bgcolor: '#FF0000', color: 'white' } }}
              size="large"
            >
              <YouTubeIcon />
            </IconButton>
