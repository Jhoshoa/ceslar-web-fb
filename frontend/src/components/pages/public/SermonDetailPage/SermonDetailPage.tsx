...
const SermonDetailPage = () => {
  ...
  const title = getLocalizedText(sermon.title);
  const description = getLocalizedText(sermon.description);
  const speaker = typeof sermon.speaker === 'object' ? sermon.speaker?.name : sermon.speaker;
  const createdAt = parseDate(sermon.createdAt);
          ...
          {sermon.scriptureReferences && sermon.scriptureReferences.length > 0 && (
            <Box sx={{ mt: 3 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {sermon.scriptureReferences.map((ref: string) => (
